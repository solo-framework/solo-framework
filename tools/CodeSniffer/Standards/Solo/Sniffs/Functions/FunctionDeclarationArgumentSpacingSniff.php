<?php
/**
 * Порядок оформления аргументов функции:
 * Не должно быть пробелов между параметрами и скобками
 * Если есть значение парамера по умолчанию, знак = отделяется пробелами
 * Параметры перечисляются черех запятую и пробел
 * 
 * PHP version 5
 * 
 * @category Framework
 ...
 * @link     nolink
 */
class Solo_Sniffs_Functions_FunctionDeclarationArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
	...
	public function process (PHP_CodeSniffer_File $phpcsFile, $stackPtr)
	{
		...
		while (($nextParam = $phpcsFile->findNext(T_VARIABLE, ($nextParam + 1), $closeBracket)) !== false)
		{
			...
			if ($nextCode === T_EQUAL)
			{
				// если следующий токен не пробел
				if ($tokens[($nextParam + 1)]['code'] !== T_WHITESPACE)
				{
					$arg = $tokens[$nextParam]['content'];
					$phpcsFile->addError("Expected 1 spaces between argument \"$arg\" and equals sign;", $nextParam);
				}
				if ($tokens[($nextParam + 3)]['code'] !== T_WHITESPACE)
				{
					$arg = $tokens[$nextParam]['content'];
					$phpcsFile->addError(
					"Expected 1 space between default value and equals sign for argument \"$arg\";", $nextParam);
				}
			}
			...
			else
			{
				...
				if ($whitespace['code'] === T_WHITESPACE)
				{
					...
					$arg = $tokens[$nextParam]['content'];
					// Before we throw an error, make sure there is no type hint.
					$bracket = $phpcsFile->findPrevious(T_OPEN_PARENTHESIS, 
					($nextParam - 1));
