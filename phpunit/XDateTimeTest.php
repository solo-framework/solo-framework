...
class XDateTimeTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$d = new XDateTime('2000-01-01');

		$ser = serialize($d);
		$deser = unserialize($ser);

		$this->assertNotNull($deser, "can't be null");

		// Объекты должны быть одинаковые
		$this->assertEquals($d, $deser);

		// и дополнительно проверим
		$this->assertEquals($d->format("c"), $deser->format("c"));

		$this->assertEquals($d->getTimezone(), $deser->getTimezone());
